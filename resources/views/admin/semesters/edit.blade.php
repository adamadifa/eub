<x-admin-premium-layout>
    <div class="flex flex-wrap items-center justify-between gap-4 mb-8">
        <div>
            <h1 class="text-2xl md:text-3xl font-bold text-gray-800">Edit Semester</h1>
    ...
    <div class="max-w-3xl">
        <div class="bg-white rounded-3xl border border-gray-100 card-shadow overflow-hidden">
            <div class="p-8 border-b border-gray-50">
                <h3 class="font-bold text-gray-800 text-lg">Ubah Data Semester</h3>
                <p class="text-sm text-gray-400">Pastikan informasi periode akademik sudah benar.</p>
            </div>
            <div class="p-8">
                <form action="{{ route('admin.semesters.update', $semester) }}" method="POST" class="space-y-8">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="p-6 bg-orange-50/50 rounded-2xl border border-orange-100/50 flex items-center justify-between group cursor-pointer" onclick="document.getElementById('is_active').click()">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-orange-600/70">Mengaktifkan ini akan menonaktifkan semester lain.</p>
                            </div>
                        </div>
                        <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $semester->is_active) ? 'checked' : '' }} 
