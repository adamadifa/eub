<x-admin-premium-layout>
    <x-slot name="title">Edit Kelas - {{ $class->name }}</x-slot>
    ...
    <div class="mb-8 flex items-center gap-4">
        ...
        <div>
            <h1 class="text-3xl font-bold text-gray-800 tracking-tight">Edit Kelas</h1>
            <p class="text-gray-500 mt-1">Perbarui pemetaan kelas untuk <b>{{ $class->course->name }}</b>.</p>
        </div>
    </div>
            ...
            <form action="{{ route('admin.classes.update', $class) }}" method="POST" class="space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-2">
                        ...
                        <input type="text" name="name" value="{{ old('name', $class->name) }}" required placeholder="Contoh: Kelas A, IF-01, Regular"
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="course_id" required class="w-full px-5 py-4 bg-gray-50 border-none rounded-2xl focus:ring-2 focus:ring-orange-500 transition-all font-medium text-gray-700 appearance-none">
                            ...
                            @foreach($courses as $course)
                                <option value="{{ $course->id }}" {{ old('course_id', $class->course_id) == $course->id ? 'selected' : '' }}>[{{ $course->code }}] {{ $course->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="lecturer_id" required class="w-full px-5 py-4 bg-gray-50 border-none rounded-2xl focus:ring-2 focus:ring-orange-500 transition-all font-medium text-gray-700 appearance-none">
                            ...
                            @foreach($lecturers as $lecturer)
                                <option value="{{ $lecturer->id }}" {{ old('lecturer_id', $class->lecturer_id) == $lecturer->id ? 'selected' : '' }}>{{ $lecturer->user->name }} ({{ $lecturer->title }})</option>
                            @endforeach
                        </select>
                ...
                <div class="pt-6 flex items-center gap-4">
                    <button type="submit" class="flex-1 md:flex-none px-10 py-4 bg-brand-orange text-white rounded-2xl font-bold shadow-lg shadow-orange-100 hover:bg-orange-600 transition flex items-center justify-center gap-2 group">
                        Perbarui Kelas <i data-lucide="save" class="w-5 h-5 group-hover:scale-110 transition-transform"></i>
                    </button>
